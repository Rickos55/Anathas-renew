...
app.get('/dashboard', requireLogin, async (req, res) => {
  ...
  res.render('dashboard', { user, country: c, alliance, unread });
});
...
app.get('/ranking', requireLogin, async (req, res) => {
  const user = await User.findByPk(req.session.userId, { include: Country });
  const countries = await Country.findAll({ include: [User, { model: Alliance, as: 'CountryAlliance' }] });
  for (const c of countries) { c.militaryPower = computeMilitaryPower(c); c.score = computeScore(c); await c.save(); }
